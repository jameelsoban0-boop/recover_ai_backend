...
async function handleDeleteAccount(req, res) {
  try {
    ...
    await Promise.all([
      ...
      BackupGuardConfig.deleteOne({ userId }),
      BackupActivity.deleteMany({ userId }),
      RestoreOrganiserActivity.deleteMany({ userId }),
    ]);
...
async function handleGetAiPrivacy(req, res) {
  try {
    const user = req.authUser;
    const aiPrivacy = user?.aiPrivacy || {};
    res.json({
      preferOnDevice: aiPrivacy.preferOnDevice !== false,
      confirmBeforeUpload: aiPrivacy.confirmBeforeUpload !== false,
      allowCloudAI: aiPrivacy.allowCloudAI === true,
    });
  } catch (err) {
    console.error("getAiPrivacy error:", err);
    res.status(500).json({ error: NETWORK_ERROR });
  }
}

async function handleUpdateAiPrivacy(req, res) {
  try {
    const user = req.authUser;
    const current = user.aiPrivacy || {};
    const updates = {};

    for (const key of ["preferOnDevice", "confirmBeforeUpload", "allowCloudAI"]) {
    ...
    user.aiPrivacy = { ...current, ...updates };
    await user.save();

    res.json({
      preferOnDevice: user.aiPrivacy.preferOnDevice !== false,
      confirmBeforeUpload: user.aiPrivacy.confirmBeforeUpload !== false,
      allowCloudAI: user.aiPrivacy.allowCloudAI === true,
    });
  } catch (err) {
    console.error("updateAiPrivacy error:", err);
    res.status(500).json({ error: NETWORK_ERROR });
  }
}
...
async function handleForgotPassword(req, res) {
  try {
    ...
    await sendEmail(
      email,
      "Reset your RecoverAI password",
      `Your RecoverAI password reset code is: ${otp}`,
      {
        html: buildOtpEmailHtml({
          title: "Reset your password",
          preheader: `Your RecoverAI password reset code is ${otp}.`,
          otp,
          message:
            "Use this one-time code to reset your RecoverAI password and get back into your account.",
        }),
      }
    );
