...
async function getBackupGuardOverview(req, res) {
  try {
    const [configuredUsers, byFrequency, recentFailures] = await Promise.all([
      BackupGuardConfig.countDocuments({}),
      BackupGuardConfig.aggregate([
        { $group: { _id: "$frequency", count: { $sum: 1 } } },
      ]),
      BackupActivity.find({ status: "failed" })
        .sort({ startedAt: -1 })
        .limit(20)
    ...
    return res.json({
      success: true,
      configuredUsers,
      byFrequency: byFrequency.reduce((acc, row) => {
      ...
      recentFailures: recentFailures.map((entry) => ({
        id: entry._id,
        user: entry.userId
          ? { id: entry.userId._id, name: entry.userId.name || "", email: entry.userId.email || "" }
          : null,
        source: entry.source,
    ...
    console.error("[admin:getBackupGuardOverview] error:", error);
    return res.status(500).json({
      success: false,
      message: "Failed to fetch backup guard overview",
      error: error.message,
    });
  }
}
