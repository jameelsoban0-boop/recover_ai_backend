...
Strict limitations — never claim otherwise:
- RecoverAI cannot recover permanently deleted data from raw internal memory with no accessible copy
- RecoverAI cannot recover files after a factory reset unless a backup already existed
- RecoverAI cannot access private data belonging to another app
- RecoverAI never guarantees recovery or repair of any file — always speak in terms of what can be checked, not what is guaranteed

Be concise, practical, and reassuring. Use plain language unless the user clearly wants technical depth.
Limit responses to 3–5 short paragraphs unless a longer explanation is genuinely needed.`;

const APP_NAVIGATION_GUIDANCE = `Important in-app navigation rule:
- If the user asks about running a scan, checking their Recycle Bin, viewing their backups, or restoring a file, do not invent results or numbers you don't have.
- The chat context intentionally does not always include full scan/backup state because sending it can slow chat responses.
- Answer naturally based on the user's wording: point them to the Scan tab to run a Smart Recovery Scan, the Library tab for Recycle Bin / Backups / Restored Files, and the Backup Guard tab to check or set up scheduled protection.
- Do not claim a scan or restore has already happened unless that information was explicitly provided to you in this conversation.`;

function buildSystemPrompt(recoveryContext) {
  if (!recoveryContext || typeof recoveryContext !== "string" || !recoveryContext.trim()) {
    return `${CHAT_SYSTEM_PROMPT}

${APP_NAVIGATION_GUIDANCE}`;
  }
...
════════════════════════════════════════
RECOVERY CONTEXT (scan/backup/library state)
════════════════════════════════════════
RecoverAI has already read the following data directly from the user's device with their explicit consent. This is real, current data — not hypothetical.
...
════════════════════════════════════════
• NEVER say you cannot access the device's scan/backup/library state — you already have all of it above.
• Treat every value above as ground truth for this conversation.
• Refer to specific details (file names, locations, backup dates, scan categories) in your answer.
• Give targeted, personalised guidance based on this exact state rather than generic tips.
• If the user asks about a specific file's status, read the answer directly from the data above using the approved status vocabulary and state it confidently.`;
}
...
async function getChatClient() {
  ...
  if (!chatOpenAiClient || chatOpenAiClientKey !== key) {
    chatOpenAiClient = new OpenAI({ apiKey: key });
    chatOpenAiClientKey = key;
  }
...
function isAmazonClient(req) {
  const header = String(req.get?.("X-Client-Platform") || req.headers?.["x-client-platform"] || "")
    .trim()
    .toLowerCase();
  if (header === "amazon") return true;
  const bodyPlatform = String(req.body?.platform || "").trim().toLowerCase();
...
async function callOpenAI(messages, recoveryContext) {
  const client = await getChatClient();

  const systemPrompt = buildSystemPrompt(recoveryContext);
  console.log("[callOpenAI] model:", getModel());
  console.log("[callOpenAI] recoveryContext provided:", !!recoveryContext);
...
async function handleGuestChat(req, res) {
  ...
  let result;
  try {
    result = await callOpenAI(payload, recoveryContext);
  } catch (err) {
...
async function handleRespond(req, res) {
  ...
  const recoveryContext =
    typeof req.body.recoveryContext === "string" && req.body.recoveryContext.trim()
      ? req.body.recoveryContext.trim()
      : null;

  console.log("[chat respond] userId:", userId, "| sessionId:", sessionId, "| recoveryContext:", !!recoveryContext);
  if (recoveryContext) console.log("[chat respond] recoveryContext:\n", recoveryContext);
  ...
  let result;
  try {
    result = await callOpenAI(history, recoveryContext);
  } catch (err) {
    if (err.statusCode === 503) return res.status(503).json({ error: err.message });
    const msg = err?.error?.message || err?.message || "OpenAI request failed";
...
async function handleVerifyIapPurchase(req, res) {
  try {
    ...
    if (!isKnownPremiumProduct(platform, productId)) {
      return res.status(400).json({
        error: "productId is not a recognised RecoverAI Premium subscription",
        entitlement: await buildEntitlement(user),
      });
    }
